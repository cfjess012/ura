...
export function SeverityForm({
  ...
  const [details, setDetails] = React.useState<Record<string, string[]>>(
    Object.fromEntries(items.map((i) => [i.question.detail?.questionId ?? i.question.id, i.detail])),
  );
  ...
  async function save(
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (only.includes(q.questionId) && nextBands[q.questionId])
          payload[q.questionId] = nextBands[q.questionId]!;
        if (q.detail && only.includes(q.detail.questionId))
          payload[q.detail.questionId] = nextDetails[q.detail.questionId] ?? [];
      }
  ...
  function toggleDetail(question: SeverityQuestion, option: string, on: boolean) {
    ...
    <form
      onSubmit={async (event) => {
        ...
        const everything = items.flatMap((i) =>
          [i.question.questionId, i.question.detail?.questionId].filter(Boolean),
        ) as string[];
    ...
    >
          ...
          <section key={question.id} className="card q2">
            ...
            {derived && !band && (
              <p className="prefill" role="note">
                ...
                <span>
                  This looks like <strong>{derived.band}</strong> because {derived.because}.
                  Pick a different one if that&rsquo;s not right.
                </span>
              </p>
            )}
              ...
              {BANDS.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="band-anchor">{question.bands[option]}</span>
                  </button>
                );
              })}
            </div>
            ...
            {showsDetail && (
              <div className="detail reveal">
                <p className="why">
                  <span aria-hidden="true">↳</span> Shown because you answered {band}.
                </p>
                <p className="field" id={`${question.detail!.questionId}-label`}>
                  ...
                  {question.detail!.options.map((option) => (
                    <label key={option} className="pathopt">
                      <input
                        ...
                        value={option}
                        checked={(details[question.detail!.questionId] ?? []).includes(option)}
                        onChange={(e) => toggleDetail(question, option, e.target.checked)}
                      />
      ...
      <div className="savebar">
        <span className="missing">
          ...
          {owed.length > 0 ? ` · ${owed.length} control${owed.length === 1 ? "" : "s"} required so far` : ""}
        </span>
