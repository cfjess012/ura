...
**Refused and unavailable are different answers.** The gate looking at a
suggestion and saying no means their writing stands; the model falling over
means nobody read it. Both used to print "Nothing worth suggesting just now
— what you wrote stands", which tells somebody their text is fine when in
fact it was never examined. The agent returns which, and the screen says the
honest one.

**Taking a suggestion moves them to the field.** The check runs at the foot
of the last section; the field it rewrites is usually on the first. So "Use
this" carries the suggestion in **session storage**, navigates to the owning
section, puts the text in the field, and selects the first bracket so typing
replaces it. Session storage, not the record: a suggestion is not an answer
until they save it, and writing it to the database on the way past would
make the platform the author of a sentence they are about to attest to. It
is read once — a second read would overwrite their edits — and per tab, so
closing the page means the suggestion was never taken.

The gaps are listed under the field, each one clickable to select it there.
A textarea cannot carry markup, so the brackets cannot be highlighted where
they sit; pointing at them from underneath does the same work and one thing
better, which is that choosing one puts the caret in it.
