...
test("the controls asked about are the ones the answers required", async ({
  page,
}) => {
...
test("submitting without the note is refused, and says which one", async ({
  page,
}) => {
  ...
  await card
    ...
    .click();
  await page
    .getByRole("button", { name: /Save and see where this stands/ })
    .click();
...
test("an answer survives leaving the screen and coming back", async ({
  page,
}) => {
  ...
  await card
    ...
    .fill("Nothing exists for this yet.");
  await page
    .getByRole("button", { name: /Save and see where this stands/ })
    .click();
  ...
  await expect(again.locator("> .q3-note textarea")).toHaveValue(
    "Nothing exists for this yet.",
  );
});

test("controls with no questions are declared, never silently dropped", async ({
  page,
}) => {
...
test("an answer needing a note is not saved until it has one", async ({
  page,
}) => {
