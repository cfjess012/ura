...
test("the front door introduces the platform and asks who you are", async ({
  page,
}) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: /One front door/ }),
  ).toBeVisible();
  await expect(
    page.getByText("Roles are enforced for real, not simulated"),
  ).toBeVisible();
...
test("choosing a person switches immediately — no second click", async ({
  page,
}) => {
...
test("switching persona changes role, navigation, and what is permitted", async ({
  page,
}) => {
  await page.goto("/projects");
  ...
  await expect(
    page.getByRole("heading", {
      name: "Every agent, and what it may never do.",
    }),
  ).toBeVisible();
  await expect(
    page.getByText("the persona switcher is a pilot device"),
  ).toBeVisible();
...
test("Switch user leaves the product and returns to the front door", async ({
  page,
  context,
}) => {
  await page.goto("/");
...
test("an answer records who gave it", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: /AI \/ ML/ }).getByText("Applies"),
  ).toBeVisible();
});
...
test("a requester sees their own assessments; a Risk Assessor sees the queue (F2)", async ({
  ...
  await becomePerson(page, "Noah Kahan");
  await expect(
    // A reviewer's landing page is a queue now, not a list of everything:
    // grouped by what is blocking each assessment, oldest first.
    page.getByRole("heading", { name: "Your queue." }),
  ).toBeVisible();
  // A draft is not in a reviewer's queue: nothing can be attested and no
  // finding exists yet. It is withheld out loud rather than omitted.
  await expect(page.getByRole("link", { name: adminOwned })).toBeHidden();
  ...
  await expect(
    page.getByText(/you don.t open one on their behalf/i),
  ).toBeVisible();
  // Withheld, not hidden: asking for them shows them.
  await page.getByRole("link", { name: /Show them anyway/ }).click();
  await expect(page.getByRole("link", { name: adminOwned })).toBeVisible();
});

test("an intake change records who made it, and says so on the screen (F5)", async ({
  page,
}) => {
  await page.goto("/projects");
  await becomePerson(page, "Priya Sharma");
  ...
  await page
    ...
    .fill("Replace a spreadsheet used for shift planning.");
  await page
    .getByLabel("Project Description")
    .fill("Shift planning replacement.");
  await page
  ...
  await expect(page.getByText(/Last change saved by/)).toContainText(
    "Priya Sharma",
  );
});
...
test("an assessment cannot be opened or edited by someone it doesn't belong to (N1)", async ({
  ...
  for (const path of [
    base,
    `${base}/intake/ownership`,
    `${base}/assess/complete`,
  ]) {
...
test("a Risk Assessor is refused in writing when they try to start an assessment (N3)", async ({
  ...
  await page
    .getByLabel("Start a new assessment")
    .fill(`Should be refused ${Date.now()}`);
  await becomePerson(page, "Noah Kahan");
  await expect(page.locator(".whoami-role")).toHaveText("Risk Assessor");
  await expect(page.getByLabel("Start a new assessment")).toHaveCount(0);
});
