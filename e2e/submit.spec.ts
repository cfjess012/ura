...
test("the declaration shows the answers being declared, not a checkbox alone", async ({
  page,
}) => {
...
test("gaps are named, and submitting with them takes a second confirmation (FR-14)", async ({
  ...
  await expect(
    submit,
    "the declaration alone is not enough while gaps remain",
  ).toBeDisabled();
...
test("submitting raises a finding carrying its note, and says so first (FR-15)", async ({
  ...
  await expect(
    page.getByText("No recertification process exists today."),
  ).toBeVisible();
  // That control is governed by a policy clause, so the finding is the
  // richer of the two kinds — a breach naming its authority, rather than a
  // bare gap. One fact, one finding (G-67).
  await expect(page.locator(".owed")).toContainText("Breaches policy");
  await expect(page.locator(".owed")).toContainText(/IAM-STD-004/);
...
test("a submitted assessment cannot be edited, and says why", async ({
  page,
}) => {
...
test("the submitted view names a person, never an identifier (NFR-9)", async ({
  page,
}) => {
  ...
  await page.getByRole("button", { name: /Declare and submit/ }).click();
  await expect(
    page.getByText(/Declared accurate by Priya Sharma/),
  ).toBeVisible();
});
