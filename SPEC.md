...
Packaging requires: every visible question attested · zero open findings · zero open conflicts. The export is **insert-only and replayable**: a structured record mapping every attested value (including explicit N-A strings) to destination fields, plus accepted scenarios, plus the coverage of what was asked and why. Re-export creates a new record; nothing is overwritten · a reading that exists at all — an assessment nothing has answered cannot be packaged, because the export would carry a rating nobody made (G-81).

### 4.6 Rating — settled (G-78, amended G-79 and G-81)

An assessment carries two ratings and a finding one, each a **band with its reasons** — Low, Medium, High, Critical — computed by the one engine from rules that are instrument data (`rating.json`, versioned like every edition) and never stored. **Inherent** reads the severity answers, the areas and the parts before anyone asks whether a control exists. **Residual** starts from the inherent band and **can only fall**: a control reduces risk, it does not create it, so a residual above its inherent band is a contradiction and the engine refuses an edition whose steps could produce one. Open findings, lapsed acceptances and overdue fixes are the reasons a reduction was **not earned**, named as such; a reduction is earned only by evidence — every required control attested, the high-weight ones in place or none applicable, and nothing open or overdue. **Inherent** reads the profile rather than the single worst answer: a whole risk area at High, two areas elevated, or the activity itself High on two or more of how critical it is, how widely it is deployed and who can reach it. The unit of judgement is the risk area and the activity's own consequence, never one answer among many, so adding questions does not drag every assessment upward. A **promised fix is a plan, not a control**: a remediation inside its date earns nothing back until a reviewer settles the finding again to say it is made. **Appetite** is data too: a band a scope may not exceed, and where it escalates when it does. **A rating carries its standing, and may have no band at all.** Where nothing has been answered that could rate an assessment the product says so, rather than falling to its lowest band: absence of an answer is not evidence of a low one. Where some of it is answered the band is a **floor** — it can rise, never fall for want of an answer — and every screen labels it provisional. **Intake is evidence** and can set that floor before a risk question is answered. No composite number is displayed anywhere; the band is the reading and the reasons are the evidence. A rating is frozen only into the package payload at packaging.
...
One condition engine evaluates: equals / not-equals / includes / excludes / answered / blank / numeric thresholds / severity-at-least, over one flat namespace — intake fields, `gate.<area>`, the parts that apply (`paths`, and `path.<area>` as its per-area alias), `severity.<question>` — with all / any / not nesting (`src/lib/conditions.ts`, S14). Everything routes through it — gates, path activation, conditional reveals, control accumulation, follow-ups. **Positive evidence holds for every operator but one:** a negative on an unanswered field is false, and `not` holds only when everything inside it is answered and false; `blank` alone reads absence, and the lint confines it to intake visibility (G-77). It renders any condition as one English sentence in the instrument’s own words with no identifier in it (`condition-text.ts`, the explainability surface), and every validator — the three JSON editions and the intake sections — lints authored conditions at import for what cannot fire or cannot fail: an option the field never offers, includes-and-excludes of one value, a rule beside its own negation, a self-contradiction, an empty group, a rule written twice (`condition-lint.ts`).
...
- **G-44 (SUPERSEDED by G-74, 2026-08-26):** Demo readiness is its own tracked artifact (2026-08-21, owner's question). Build completeness and demo readiness are different things; `demo/readiness.md` holds one row per beat — what the room sees, what delivers it, built or not, **walked by a person or not**, and the fallback if it breaks live — plus what will not be claimed. The Stop gate refused to finish while `slices-covered` lagged the DONE slices. **A beat nobody has used is not ready, however green the tests are** — the sentence this entry existed for, and the one G-74 keeps after retiring the artifact.
...
- **G-74 (settled):** The demo apparatus is retired (2026-08-26, owner call), superseding **G-44**. The leadership demo was given on 2026-08-25. `demo/readiness.md` and `demo/three-minutes.md` are deleted, with `scripts/walk-demo.mjs`, `test/unit/demo-readiness.test.ts`, the `walk:demo` script, and the two Stop-gate checks that read the readiness file — the one refusing to finish while `slices-covered` lagged the DONE slices, and the one requiring a stated measurement to cite an existing test. **What is being retired is the audience, not the honesty.** G-44's rule was that build completeness and demo readiness are different things, and it was right for as long as there was a room; with the room gone, a file tracking what the room sees is ceremony that would rot in place, and a stale artifact the Stop gate enforces is worse than none — it teaches that the gate can be satisfied by editing a document. **The claims survive the artifact.** `demo-truth` remains a loaded skill and is now the *only* thing checking that a number this project states about itself is a number something computes: the mechanical citation check went with the file, so the pass that was a backstop is now the front line, and it says so. The measurements themselves stay asserted in `prefill-reach.test.ts` — four intake answers decide four of eleven areas; depth exists in exactly four risk areas — because they are facts about the instrument that were only ever *reported* by the demo. The pilot's depth boundary is declared to a person on screen (FR-35), which is where G-50 always wanted it, never in a document a presenter held. **The general rule: a governance artifact outlives its purpose silently.** G-44 was correct on the day it was written and became ceremony the moment the demo was over; nothing in the build would have said so, because a green gate reads the same whether it is protecting something or not. An artifact created for an event is retired with the event, by name, in this log.

- **G-75 (settled):** The process is drawn as a map an administrator can walk (2026-09-02, owner request: "a beautiful plain-English interactive 3D model of the risk assessment process, conditions and logic — the full end-to-end model, noting any artifacts — for the admin persona, and the same for the risk assessor"). `/admin/model`, FR-48. **Three decisions.** First, three-dimensional by CSS transform, not WebGL: a canvas would add a heavy client dependency and give a screen reader nothing, while a transformed DOM is a real list of real buttons that reads in order — the same rule as every other surface (§23). Second, **every number on the map is computed from the instrument**, never typed — the demo-truth rule applied to a screen that exists only to make claims about the product; `process-model.test.ts` holds each count to its source. Third, the sentences are the product’s own claims about itself and are held to the same standard as any screen: each describes a thing that runs today, and where the pilot stops the map says so as a stop of its own (G-50). Routes are not shown — a route pattern is an internal identifier (NFR-9); each stop says which screen in plain words.

- **G-76 (settled):** Seven defects found by the 2026-09-02 evaluation are fixed before anything is built on them (S13, "debts first"). (a) The reviewer's queue counted any disposition row as a settlement, so an **expired risk acceptance vanished from the queue and the bell** while the review, the report and the packaging gate showed it open — the one-rule design of `findingIsOpen` defeated by raw SQL it could not reach; the SQL now states the rule again, in the one place it cannot be called, and the architecture test holds it there. (b) The hand-off thread cap `MAX_DEPTH` was declared and never enforced — `depthOf` ignored its argument. (c) "An approval owes a note" (2026-08-25) was enforced in TypeScript only; it is a CHECK now, `NOT VALID` so the approvals recorded before the call stay as written. (d) The controls this activity requires but the pilot never asks about were declared on the objectives screen and **absent from the report and the package** — the artefacts that outlive the screen now name them with their reasons. (e) `answers.value` was typed as a string or a list while Tier 3 stores an object, cast through `unknown` at the write. (f) Intake visibility re-implemented the three operators the header said it routed through `matches()` — a second evaluator, §3.3's defect by definition. (g) `remediation_due` was stored and read by nothing; a promised fix past its date is now **overdue** in the queue, the review, the report and the package (`findingStanding`), and the review offers the settlement a decision needs. **The rule this entry keeps:** a rule with one definition is only worth what reaches it; every reader of the record, raw SQL included, is held to the pure function by a test, and a date the product stores is a date the product reads.

- **G-77 (settled):** The condition language is complete to §6.3 and keeps positive evidence for every operator but one (2026-09-03, S14). `notEqualsAny` and `excludesAll` on an unanswered field are **false**, and `not` holds only when everything inside it is answered and false — because a negative that fired on silence would let a rule waive a control by somebody leaving a box empty, the exact thing §3.2.1 exists to forbid. `blank` is the one operator that reads absence, and it is confined by the lint to intake visibility: a note may appear because a box is empty; a risk area, a part, a requirement or a follow-up may never turn on one. **The lint runs at import, in every validator**, so an instrument holding a rule that cannot fire (an option the field never offers, a typo in a field name, `blank` where it is refused) or cannot fail (a self-contradiction, an empty group) does not boot. The renderer speaks only the instrument’s words: a field it does not know is "a question", never an id (NFR-9). The four operators every shipped rule uses behave as before with one named change — a single answer satisfies includes/excludes as a list of one, the scalar case §19 names — and `conditions.test.ts` holds them to it. The intake sections, a TypeScript literal, get a validator of their own so their reveals are linted at import like the JSON editions.

- **G-78 (settled):** The assessment is rated — bands with reasons, never a number (2026-09-03, owner decision, S15; closes §14.1 and rewrites §4.6). The URA’s no-composite stance was right about the number and wrong about the reading: a reviewer sorting a queue and a leadership audience reading a package both need to know how risky an activity is, and "twenty-six bands across five screens" is not an answer. So there are two ratings, each a band with every reason that holds, computed by the one engine from rules that are **instrument data** (`rating.json`, a versioned edition the S14 lint refuses when a rule cannot fire). **Inherent** reads the severity answers, the areas and the parts; **residual** starts from it and moves by whole bands for an open non-compliance, an open gap on a high-weight control, an overdue fix, and falls only when every required control is attested and the high-weight ones are in place — a lapsed acceptance reopens its finding and the rating follows. **Appetite** is data too, per scope, with where a breach escalates; it is a routing signal and never a gate. Every rule is a condition over a namespace this slice derives (`worst.severity`, `areas.atHigh`, `area.<key>.severity`, `findings.*`, `controls.*`) merged with the assessment’s own answers, so the renderer can say any rating rule in words and there is still one evaluator. **The first edition’s rules are the owner’s to ratify** — they are the reading, and the reading is a governance act. `grounding.ts`’s review band is unchanged: it grades evidence, this grades risk, and the two never mix. **Amended 2026-09-03 (edition `risk-rating@2026-09-03.2`, after the S15 verifier's FAIL):** a reason must be true of the record it describes — the Medium rule's reason said "is Medium" for a condition meaning Medium or above, and now says so; the rating reads only the severity answers the assessment is currently asking, so unticking a part takes its answer out of the inherent and area bands without deleting it (§19); a rule's own fields — its band, its step as a whole number of bands — are refused at import alongside its condition, because ratifying an edition is an edit by someone who will not be reading the code; a breach carries its scope in words and where it escalates into the package.
- **G-79 (settled):** The rating is bounded and reads the profile (2026-09-03, owner decision after a risk-management review of the first edition; amends G-78, edition `risk-rating@2026-09-03.3`). Three things were wrong against ISO 31000, COSO ERM and NIST SP 800-30, and two are now fixed. **Residual could exceed inherent** — an open finding pushed the band up, so an assessment at High landed at Critical after controls. Controls reduce risk; they do not create it. Every residual step is now zero or negative, the engine clamps at the inherent band, and `validate()` refuses an edition with a positive step, so the invariant cannot be edited away. What used to raise the band now names, in the same words, why nothing was earned back — and a reduction must be earned by evidence rather than by an absence of findings, because an assessment requiring no control at all was falling a band for having demonstrated nothing. **The worst-answer roll-up did not scale** — the band was the maximum over twenty-six answers, so every question added raised the chance that one was High and the distribution converged on High and Critical, which sorts nobody; the unit of judgement is now the risk area (a whole area at High, or two areas elevated). **Likelihood is still missing** and is not fixed here: the severity set asks consequence and exposure only, so what the engine computes is an impact tier, not risk in the ISO 31000 sense. That is instrument content, recorded as an open question in §14 and owed by S17. Portfolio aggregation and risk capacity remain S22's. **Delta-verified and extended the same day (edition `.4`):** a high-weight control a reviewer formally ruled Not applicable no longer counts as one the assessment requires, which had left exactly the assessments G-79 set out to help unable to improve; the edition now names the commonest reason a reduction was not earned, a control still waiting for a signature, which the screen had been silent about; the acceptance line no longer says "the rating holds" on a record whose rating had just fallen; and the package's provenance carries the rating edition beside the editions that asked the questions, so a replayer can tell which rules produced the frozen bands. **Both gaps that verification raised are closed too (edition `.5`):** the three questions asked of every assessment — how critical the activity is, how widely it is deployed, who can reach its output — describe consequence, which ISO 31000 and COSO treat as first-class, and belong to no risk area, so the area-based roll-up had left them unable to move the band at all; they now carry it on corroboration (two of the three at High reaches High, and with a risk area also at High, Critical), never on one answer. And a remediation inside its date no longer earns the reduction, because a plan is not a control; the reviewer releases it by settling the finding again once the fix is real, which the existing dispositions already allow.
- **G-80 (settled):** Framework content is transcribed, licensed correctly, and dated (2026-09-03, owner decision, S16). **Nothing is written from memory.** A model can produce a clause number and a paragraph that read perfectly and are wrong, and a fabricated ISO reference is invisible to every test here and to the person reading it — the exact defect this product exists to prevent in other people's work. So each framework is transcribed from its publisher's own machine-readable source by `scripts/fetch-framework.mjs`, and the file records the source URL, the edition, the retrieval date and who checked it. **Public frameworks only are shipped** — NIST's, GDPR, the EU AI Act — because ISO and the SOC 2 criteria are licensed and their text may not be redistributed; the owner holds no licensed copies, so rather than invent them the product ships a loader and an organisation adds the framework it is licensed for (FR-54), which is also how it adds its own internal control set. A framework marked not quotable may carry references and headings but no text, enforced at import. And every framework carries a review window: past it the build fails naming the command that re-transcribes it, because a crosswalk that was right in 2026 is quietly wrong in 2028 unless something asks. The Secure Controls Framework was investigated as a route to the licensed standards and is **not** relied on: its own FAQ is ambiguous about the Creative Commons variant and mentions commercial licences for derivative content, which reshaping its mappings would be. That is a legal question for the owner, not an engineering one.
- **G-81 (settled):** A rating carries its standing, and intake is evidence (2026-09-03, owner decision after a smoke test of a real record; amends G-78/G-79, edition `risk-rating@2026-09-03.6`). A submitted assessment whose intake said it uses AI, involves a company outside ours, handles Confidential data and contains applicant personal information, and which had **no other answers at all**, rated **Low** on the reviewer's queue, the report and the submitted view. Nothing was wrong with the arithmetic: with no answers no rule fired, so the rating fell to its default band, which is Low. The defect is that **the scale had four values and no way to say "nobody has assessed this"**, so absence of evidence rendered as evidence of absence on the screen a reviewer triages from. Three changes. A rating now carries a **standing** — rated, provisional, or unrated with **no band at all**; the type makes the band nullable so every screen is forced to say what it shows rather than quietly printing Low. **Intake can establish a floor**, because what a person tells the front door is evidence: confidential data or above reaches Medium, information about identifiable people in an activity involving AI reaches High, and the most sensitive data with a third party reaches High. A floor can only rise, and is labelled provisional wherever it appears. And **an unrated assessment cannot be packaged**, which amends §4.5: with no answers there is no unattested control and no open finding, so such a record passed every other condition and would have exported a reading nobody made. Submitting with everything unanswered stays allowed (FR-14 is unchanged): refusing does not produce careful answers, it produces fast invented ones, and a blank is auditable where a guess is not. Noted alongside: the intake coherence check that would have caught this record — the tool named ChatGPT, the vendor Databricks, the use case summarising 10-K filings, the data applicant personal information — was dark because the configured model key was invalid.
- **G-83 (settled):** A control says who can answer it, and platforms are assessed once (2026-09-03, owner decision). Measured first: of the fifteen control questions a requester faces, **ten are common controls about the enterprise estate** — is multi-factor authentication enforced, is privileged access separated — that no requester can answer. Two are about their own activity. The proof it stops people was already in the queue: an assessment submitted with thirteen questions unanswered by someone who reached exactly those. So each of the 51 controls is classified by NIST's model — **common** (provided centrally, inherited by everything that uses the provider), **hybrid** (part central, part the activity's), **system-specific** (only the people doing it can answer) — which comes out 18 / 9 / 24, and the system-specific two dozen are almost entirely AI, privacy, compliance and third party, which is exactly the half a risk assessor should be probing rather than a requester guessing at. Platforms are then assessed in their own right, each with an accountable **owner** who attests quarterly and who is responsible for spotting new features; each states what it is for, which controls it provides, and **what data it is cleared to hold**. An activity says which platforms it sits on, and what those platforms provide is shown with the provider and the date — never silently, which §22.1 already requires. Being behind enterprise identity and the sanctioned cloud tenancy discharges thirteen of the fifteen questions. Two rules carry the weight: a platform may only provide a control that is **centrally providable**, refused at import (this caught a real error in the first data written under it); and an attestation has an **expiry**, past which what it provided stops counting — inheritance that outlives its evidence is how control failures get laundered, and the product already applies that rule to an expired risk acceptance. Owners are placeholders until real sign-in exists (S21), because a platform owner is an accountable person, not a persona.
- **G-91 (settled):** The standing screen states where you are and offers one thing to do (2026-09-04, owner report from a screen recording). *"It seems very wordy to an end user… it appears very convoluted."* Measured: **1,067 words, 45 bullets, 3.1 scrolls** at the moment a person wants to finish — every part of it true and traceable, which is why it grew, and together it read as the system explaining its own workings. It is now **217 words on one screen**: four status rows and exactly one next action, that action being whichever step is unfinished first (§24.2). Nothing was deleted — the risk-area map, the control derivation and the after-submit explanation moved behind closed disclosures, because a reviewer and an auditor want them and a requester finishing their work does not. `assess-standing.ts` decides all of it, and the rail reads the same derivation. **Building it exposed three contradictions the old page had been hiding, none of them introduced by the redesign:** the rail said "5 answered" meaning five AREAS beside the page's "19 rated" meaning nineteen QUESTIONS; the controls row said "10 required" beside a disclosure saying "34 controls" (34 are required, 10 have questions in the pilot, 24 go to a reviewer as they stand); and the rail counted the ten areas a person is asked against the page's eleven that exist. Every number now names its unit and shares its denominator, and a browser test reads both surfaces and fails if they disagree — the guard that was missing, since nothing had ever compared them. Two rules were deliberately kept on the FACE of the screen rather than behind disclosure: the boundary (how many areas are recorded for a reviewer only, G-50) and the control split, because somebody who never expands a section would never learn either. Also fixed in passing: every filled `.btn` was rendering underlined, because most of them are links.
- **G-90 (settled):** No screen may be nothing but a redirect behind a spinner (2026-09-04, owner report). Starting an assessment redirected to `/projects/<id>`, a route whose entire body is redirects — it works out where a person belongs and forwards them — sitting behind a loading state reading *Opening the assessment…*. So creating one took two hops and a spinner to reach a form whose address was already known, and the owner met the second hop as a blank page with the URL stuck on the project root. They reasonably read it as "I submitted and it hung"; **nothing had been submitted**, which is the worse half — a screen that stalls silently lets somebody believe an act completed. Creating now goes straight to the first section (297ms, no loading state shown at all), and the redirect route's wait stops pretending to be progress after six seconds: it says nothing was being saved there, and offers a retry and the way back. **The honest limit:** the original hang was never reproduced — a hard load and a click-through both resolve correctly, and no source file was touched in the minutes around it, so a recompile does not explain it either. What is fixed is the shape that made it possible and made it silent, not a proven cause. The environment is a standing suspect and is recorded as one: the machine was running the pilot's dev server continuously since 25 August alongside the retired platform's server on :3000, which SPEC G-8 says must never run.
- **G-89 (settled):** A save may never fire from inside a state updater (2026-09-04, defect found by the owner exploring the built screen). The controls form called `autosave.save` from inside its `setGiven` updater. React runs updaters **during render**, so a failed save asked the router to redraw mid-render and React refused outright: *cannot update a component while rendering a different component*. The pattern predates the register; the drawer changed the render timing enough to surface it. Edits are now sequenced through a ref and rendered through state, with the save called as ordinary event-handler work. Every other component using `useAutosave` was checked and is clean. **The part worth keeping is how it was caught and how it is held:** it typechecked, and every existing assertion passed while it was live — the only evidence was a browser console line. A browser test now answers a control, changes the answer and writes a note, and fails on any React complaint about updating during render; the old code was deliberately restored to confirm the test fails against it, because a guard nobody has seen fail is decoration.
- **G-88 (settled):** The register holds still, and the drawer is why (2026-09-04, built under G-87). The controls screen is now one table beside a drawer. Two things were decided by building it rather than by drawing it. **Rows sort by name and nothing else.** They first sorted by state, so what needed doing came first — which read well on a screenshot and was wrong in use: answering a control changed its state, which moved it, which moved every row beneath it. That is the same "item jumps out from under the cursor" defect the platform picker was designed to avoid, and a browser test found it by reopening "the first control" and getting a different one. The family heading carries how the family stands; the list stays put. **Every row opens, including the ones nobody can answer here.** A control the pilot asks no question about used to sit in a bullet list at the foot of the page; it is now a row whose drawer says the pilot has no question for it yet, that it goes to a reviewer as it stands, and that nothing is missing from the requester's side — because "why is this on my list and why can I not act on it" is a question the screen should answer rather than leave to a guess. Also settled: the controls screen opts into a wider shell (`main.main-wide`, 1480px), because a register is a wide object and 1180px left the table four columns' worth of headings in a column too narrow for one of them.
- **G-87 (settled):** One control register, and six states rather than three presentations (2026-09-04, owner decision on the layout). The controls screen showed the same assessment's controls three unrelated ways on one page — a "covered by your platforms" list, a stack of tall question cards, and a bullet list recorded for a reviewer — each computing its own idea of what state a control was in. Nothing could see the whole set, and nothing guaranteed the three agreed. `control-register.ts` is now the single definition, and it names **six** states where the screen previously distinguished three: inherited, answered, **gap** (a Partial or a No, which is already a finding and today reads identically to a Yes until submission), to answer, **lapsed** (covered by a platform whose attestation ran out, which previously came back as a question with nothing saying why), and no question yet. Two things follow. The 13 control families gained **plain names and an owner's order** (`control-domains@2026-09-04.1`) because their codes are internal and may never reach a screen (NFR-9), and alphabetical order would put the rarest family above identity on every assessment; the validator now refuses a family whose name is its own code. And `owed` counts only what the person in front of it can act on (§24.9) — a control the pilot asks no question about is required and recorded, but counting it as outstanding would show a total nobody can drive to zero. **The register makes one measurement visible that was not:** on a realistic assessment, 7 of 15 required controls have no question yet. That is the honest size of the S17 content debt, and it is the argument for the register landing before another thirty-six questions are written. The owner chose a drawer beside the register over expanding rows, and NIST 800-53 for the reference column over the CSF refs already carried — 800-53 is a transcription still owed, and the register ships without that column until it lands.
- **G-86 (settled):** The platform records are readable, and nobody can attest on a screen (2026-09-04). The estate now has two administration surfaces: every assessed platform with its owner, whether the attestation is in force or lapsed, what it provides and **how many assessments are leaning on it**; and one platform's own record with its ceiling, its controls crosswalked to the frameworks, and the assessments that named it. The dependants column is the blast radius the plan said to build early rather than late, and the reason holds: quarterly attestation with nobody able to see what depends on them is a calendar reminder, not a control. The lapsed banner counts **assessments that became unsupported**, not rows that turned red — a person needs to know what stopped standing up, not how many dates passed. Both are **read-only, and say so on screen**: an attestation is a named person putting their name to a control, and this pilot signs people in with a switcher, so a button here would write somebody's name onto an assurance they never gave. Attestation waits for real sign-in (S21), which is the same dependency the plan already named.
- **G-85 (settled):** An offer is not an answer, and a clearance is a ceiling (2026-09-04, built under G-83). The inheritance screen shipped, and building it produced two decisions worth binding. **First:** ticking a platform originally removed the controls it provides from the question list straight away, which left a control neither asked nor answered if nobody accepted the offer — a silent gap, worse than either state. A control now leaves the list only when its own recorded answer *is* the inherited one, matched against the provenance note the platform wrote; every covered control reads **Offered** or **Recorded** on screen, and a person who answers it themselves keeps their own answer. **Second:** the compatibility check treated a platform's allowed classifications as a set, so putting Public data on a warehouse cleared to Restricted raised a warning. A clearance names a ceiling and everything at or below it is fine; the check now compares rank and the message names the ceiling. That one cried wolf on the first real screen it was pointed at, and a warning people learn to ignore is worse than no warning. Also settled here: the panel appears only when some control the activity requires is one a platform could centrally provide, because offering to discharge nothing is noise dressed as help.
- **G-84 (settled):** Sensitivity is not only about people, and confidential data alone is not a signal (2026-09-03, edition `risk-rating@2026-09-03.7`). Both intake questions about sensitivity pointed at people, so an activity whose real exposure was a pricing algorithm, privileged material or an unpublished filing had nowhere to say so — the code assistant on the pricing engine reached High only because its requester happened to tick Restricted rather than Confidential, and the crown jewels would otherwise have been assessed as ordinary. The question now asks what kinds of sensitive information are involved and offers three categories beyond personal data, and the rating reads them: such information is Medium on its own and **High once it leaves our boundary** to a third party or into AI. Separately the bare confidential floor is gone — nearly all enterprise work is confidential, so a rule firing on it alone lifted almost everything; it now lifts in combination with AI or a company outside ours, and the top classification still stands alone. **Recorded because it corrects a claim I made in the same session:** the queue evaluation attributed fifteen-of-twenty-five at High to that confidential floor. Back-testing the new edition across the twenty-five moved **nothing**, and reading the chip showed the band was coming from "information about identifiable people is used in an activity that involves AI" — a rule that is already combinational and worth keeping. The distribution may simply be honest, and if it is, discrimination has to come from the residual and the reasons rather than the inherent band. That correction is what the `impact-analysis` skill exists to produce.

- **G-71 (settled):** The intake check grades the **whole intake against a graded rubric**, five criteria scored 1–4 (2026-08-24, owner's rubric). It reads every field as one document, because **coherence is a property of the set**: a description saying "internal tool" beside a data section listing external recipients is only inconsistent when you read both — and Internal Consistency, which is the one thing a person cannot check for themselves because they know what they meant, can only be judged across fields. The five criteria and their sixteen anchors are the owner's, verbatim, in `src/data/reference/intake-rubric.json`; the reasoning behind each lives in the `intake-rubric` skill. **Two criteria are called out first when thin — Data Access and Data Sensitivity — because those decide which risk areas open and who reviews it: a thin answer there is a wrong routing, not a vague one.** The score is reported as a band with its meaning beside it and never as a bare number, and every ask carries the anchor full marks would have met. **The owner's notes suggested blocking submission below a level 3 on the two routing criteria; that was deliberately not built.** G-69 holds — a quality assistant that blocks has become a gate, the mission is reducing friction, and making it a gate is a governance decision rather than a default. The button says "Save & run AI check" and does both: the check reads the record, so without saving first it graded whatever was there before and told a person their visible text was "too thin to work from". Found by running it.

- **G-70 (EXPIRED 2026-08-26, closed):** Demo-push mode (2026-08-24, owner call). The demo is tomorrow and there is a day of features to build, so the ceremony around each change is suspended in favour of throughput. **What still runs on every change, because it is cheap and catches real things:** `pnpm typecheck` and `pnpm test:unit` — about thirty-five seconds together, covering 630 tests including every schema invariant and every guardrail. The Stop gate already enforces exactly this and is unchanged. **What is suspended:** the full end-to-end suite per change (seven minutes at best, and it was taking fifteen under load) — it runs **once at the end of the day** instead; the independent slice verifier, which costs twenty minutes and an agent; a UAT record per slice, batched into one record after the demo; and any second verification pass. **What still runs before the demo, without exception:** `pnpm walk:demo`, because it is the check that the run sheet matches the product and it has caught a stale run sheet twice, and one full `pnpm e2e`. **The risk, stated rather than glossed:** the verifier found seven blocking defects in the last two days that the unit suite did not — an unconfirmed proposal counting as a real answer, a transparency page asserting no AI was running while it was, two bypassed authority checks. Suspending it does not make those less likely, it makes them less likely to be *found*. That is the trade being made deliberately, for one day, by the person who owns the consequence. **This entry expired on 2026-08-26 and is closed.** The full chain is the rule again. **The verifier pass it promised over everything built under it has not been run, and is owed** — recorded here rather than in a plan, because a debt named in an expired entry is a debt nobody reads.
...
1. ~~**Composite scoring**~~ — **closed 2026-09-03 (G-78, S15):** ratings exist as bands with reasons, computed from data by the engine, shown wherever a person decides; a bare number never appears.
2. **Likelihood (§4.6, G-79)** — the severity set asks consequence and exposure only (criticality, breadth, audience, data classification, provider access), so the inherent band is an impact tier rather than risk as ISO 31000, COSO ERM and NIST SP 800-30 define it, which is a function of consequence **and** likelihood. Options: leave it as an impact tier and say so plainly on screen and in the export; or add a likelihood dimension to the instrument (threat exposure, frequency of use, prior incidents) and make the band a matrix of the two. The second is the framework-aligned answer and is instrument content, so it belongs to **S17** and needs the owner's ratification before the questions are written.
3. **Risk capacity and the portfolio view (§4.6, G-79)** — appetite today is a per-assessment escalation threshold. COSO separates appetite, set at entity level, from tolerance around an objective, and neither is meaningful without aggregation: many assessments individually within appetite can exceed what the organisation can absorb. **S22** builds the register and dashboards; the capacity question, and whether appetite differs by business unit or risk category, is owed before it.
4. **Tier-3 audience** — requester-completes vs. control-SME persona; affects reviewer groups only, not structure. G-50's pilot depth (four areas) narrows who the SMEs would be.
5. **History posture at handoff** — keep full git history or squash to a clean initial commit.
6. **Attachment retention posture (§3.6)** — how long uploaded bytes live, who may purge, what an export carries. Owner decision; blocks S4.6. **Narrowed 2026-08-23 (G-66):** the pilot reads documents and keeps only the **extracted text**, scoped to one assessment, with no binary store and no download path — so document-assisted drafting ships without waiting on this. The decision still gates keeping **originals**, which is what an S3 bucket would hold; `src/lib/documents.ts` is the single module that would grow one.
7. ~~**The Bedrock model-access request**~~ — **closed 2026-08-23: the owner already has Bedrock access in the sandbox account.** The plan's only uncontrollable-latency dependency is gone, which is why the Phase-2 epic starts now rather than after the remaining Phase-1 slices.
...
- **Hooks** (wired in `.claude/settings.json`, scripts in `scripts/hooks/`, wiring asserted by `test/unit/hooks-wired.test.ts`):
  ...
  - *stop-gate* (Stop) — refuses to conclude on: red typecheck/unit tests, stale generated artifacts, a governance entry that has vanished since the last commit (the backstop no tool choice can walk around), or a DONE slice without its `uat/` record (required sections with substance, including the verifier's verdict for records from 2026-08-23). It no longer reads a demo artifact (G-74).
...
| **S2** | Gates | DONE | Schema for instrument + answers with §5 CHECKs; the 11 category gates; instrument as seed data from day one; the generated agent inventory (FR-24). | FR-3, FR-22, FR-24, FR-48 · NFR-1, NFR-8, NFR-11 | Gate answers persist; No closes its category; migration tests green. |
...
| **S3.5** | Destinations · AI Use Case Record | BUILT 2026-08-27; review open | The ServiceNow AI Use Case Record assembled from answers already given, field by field with provenance and gaps; payload downloadable; **the write is deliberately not built**. | FR-26, FR-27 · NFR-20 | "Yes" to AI offers registration with a real count of fields already answered; nothing claims to have been sent. |
...
| **S9** | Package & export | DONE | Packaging gates; insert-only replayable export with explicit N-A strings. | FR-19, FR-20 | §19 packaging criteria; full-journey E2E green. |
...
| **S13** | Debts first | BUILT 2026-09-02; review open | Seven defects from the 2026-09-02 evaluation (G-76), each under the requirement it broke: the queue’s open-finding SQL restated the one rule; the hand-off thread cap enforced; the approve-note CHECK; recorded-only controls carried into the report and the package; the answer column typed truthfully; intake visibility through the one predicate; a remediation past its date read as overdue in the queue, the review, the report and the package. | — | Gate chain green; an expired acceptance reads open in the queue, the review and the package alike; an overdue fix is named in all three. |
| **S14** | The engine to §6.3 | DONE 2026-09-03 (verifier FAIL then fixed) | The full condition language with nesting, one predicate; the English renderer and the instrument vocabulary; the lint at every validator’s import. Backward-compatible with every authored rule. | FR-49 · NFR-23 | Every operator and nest tested; the shipped instrument, intake reveals and Tier-3 follow-ups lint clean; a rule that cannot fire fails the build with a sentence. |
| **S15** | Risk rating | DONE 2026-09-03 | `rating.json` as a versioned edition; inherent, residual, per-area and appetite as pure derivations over a namespace the engine reads; the band with its reasons on the ledger, the controls screen, the submitted view, the review, the dossier, the queue and the package. | FR-50, FR-51 | Every rule is data the lint accepts; a High severity and a No show High → residual with the finding named; a lapsed acceptance moves the residual; the package freezes the rating; no number on any screen. |
| **S16** | Control library & framework crosswalks | IN PROGRESS 2026-09-03 | Every control objective mapped to the framework requirements it satisfies, with the relationship named and the published wording as its basis; framework content transcribed from its publisher by `scripts/fetch-framework.mjs`, never written from memory; two coverage reports; and a loader so an organisation can add a framework it is licensed for. | FR-52, FR-53, FR-54 · NFR-24 | A mapping resolves at both ends or the build fails; a quote on a licensed framework fails the build; a framework past its review window fails the build; coverage is computed from the mappings, and a framework nobody has loaded says so on screen rather than reading as absent. |
...
**Risk rating (S15)**
- A rating is a band with every reason that holds — never a number; every reason shown is true of the record it describes (a rule whose condition is "Medium or above" says so).
- The rating reads only the severity answers the assessment is currently asking: unticking a path removes its answers from the inherent band and from that area's band without deleting the stored answers, and the ledger, the chip and the queue row agree on the same project state.
- The residual starts at the inherent band, moves by whole bands, and **never exceeds it**; every step in an edition is zero or negative, and the import refuses one that is positive.
- An assessment nothing has answered has **no band** anywhere and never reads as Low; a partly answered one reads as a floor and says so; intake alone can set that floor, and the reasons name what in the intake did it.
- A reduction is earned, never assumed: an assessment that required no control, or that has anything open or overdue, holds at its inherent band.
- One severity answer belonging to no risk area cannot carry the whole assessment; the inherent band rises on a risk area at High, on two areas elevated, or on the activity itself being High on two or more of its own counts — corroboration, never a single answer.
- A promised fix earns nothing: a remediation inside its date holds the band and says so, and only a further settlement recording that the fix is made releases the reduction.
- The edition is refused at import for a rule with no band, a step that is not a whole number of bands between -3 and 0, an empty reason, an appetite line on a scope that does not exist, or one that escalates to nobody.
- The package freezes the rating at packaging with its reasons and where any breach escalates; nothing else stores a rating.
...
| FR-48 | An administration page draws the whole assessment as a map a person can walk — every screen as a stop, in the words of the person on it, with the rules that decide what happens and what it leaves behind — for the requester’s journey and the risk assessor’s; every number on it is counted from the instrument, never typed | §22.3, §24.8, G-75 | S2 |
| FR-49 | The condition engine evaluates the full §6.3 language — equals, not-equals, includes, excludes, answered, blank, numeric thresholds, severity-at-least — with all / any / not nesting, one predicate for every routing decision, positive evidence for every operator but `blank` | §3.2.1, §3.3, §6.3, G-77 | S14 |
| FR-50 | The assessment carries an **inherent** and a **residual** risk rating — Low, Medium, High or Critical — each a band with every reason that holds, computed by the one engine from rules held as versioned instrument data (`rating.json`); the residual moves by whole bands for open findings, lapsed acceptances, overdue fixes and full attestation; nothing is stored, and the rating is frozen only into the package | §4.6, §3.2.7, G-78 | S15 |
| FR-51 | **Appetite** is instrument data — a band a scope (the assessment, or one risk area) may not exceed, with a reason and where it escalates — and every place a rating exceeds it says so, as a routing signal and never a gate | §4.6, G-78 | S15 |
| FR-52 | Every control objective carries the **framework requirements it satisfies** — the framework, its edition, the reference, the relationship (equivalent, partial or supporting) and why — and the published wording is shown as the basis wherever the licence allows it; a mapping whose control or whose requirement does not exist fails the build | §28, G-80 | S16 |
| FR-53 | Two **coverage reports**, computed from the mappings and never asserted: which requirements of a loaded framework no control addresses, and which controls cite no obligation at all | §28, G-80 | S16 |
| FR-54 | An administrator can **load a framework the organisation is licensed for**, validated by the same rules as a shipped one; a framework nobody has loaded is named on screen as a declared boundary rather than silently absent | §28, G-80 | S16 |
| NFR-24 | Reference content states its **provenance and its age** — publisher, edition, source URL, when it was transcribed and by whom, when a person last confirmed it — and a framework past its review window fails the build rather than going quietly wrong | §28, G-80 | S16 |
...
| FR-47 | A risk area explains **why it is asking what it is asking** on request — joining the rules that lit each part to the answers given elsewhere in the assessment. The rules are supplied, never inferred: the platform decides what is lit and this only explains it, so a part lit by a rule the explanation was not given is reported as unexplained rather than reconstructed. Never load-bearing — with no agent, a slow one or a refused answer the deterministic reasons stay on screen unchanged | §22.1, G-65 | S12 |
| FR-46 | A document handed to the assistant on an intake screen **drafts the activity description**, using only what the document says and marking anything it does not with a bracketed question rather than a plausible sentence. It is never saved on their behalf: the draft is shown, and taking it puts the text in the field for them to edit and attest to, because a description is signed by the person whose assessment it is | §7, FR-22, FR-40, G-66 | S12 |
| FR-45 | The conversational assistant answers a definitional question from the organisation's own standards — the clause quoted **verbatim** with its policy, reference and version, and never a paraphrase. A policy may define what a term means and what is required; it may **never assert a fact about this project**, so the reply ends with the question only the requester can answer. Where no clause covers it, the assistant says so and answers plainly rather than implying an authority it does not have. Retrieval is deterministic and runs every turn; returning nothing is the ordinary case | §22.5, G-67, G-73 | S12 |
| FR-44 | The assistant proposes an answer to the risk-area question **on the screen in front of them**, drawn only from what the requester wrote at intake and carrying a verbatim quote of it. The screen bounds what may be proposed and the record bounds what is still open; a proposal is an answer nowhere until an explicit accept writes one in the owner's name (source `person`) | §7, §22.2, FR-22, G-66 | S12 |
| FR-43 | The **whole intake** is graded against a published rubric — five criteria scored 1–4 over the set, because coherence is a property of the set and internal consistency cannot be judged field by field. A heuristic floor runs with no model; a model assigns levels and nothing else; the band, the wording and the ordering are deterministic. Every ask shows the anchor it was scored against, so a grade is readable rather than authoritative. It **never blocks submission** — no agent, a slow agent, a wrong agent or a partial answer all pass | §22.1, G-69, G-71 | S12 |
...
| NFR-23 | Every authored condition renders as one English sentence in the instrument’s own words with no identifier in it, and every validator lints authored conditions at import for rules that cannot fire or cannot fail | §6.3, §8, NFR-9, G-77 | S14 |
...
| S12 | **Document-assisted drafting** | Reads a document the requester hands over — a spec, a vendor overview, a contract — and proposes the activity description plus intake answers, each carrying the sentence it came from. What the document does not say is bracketed and named, never filled in. | A proposal is not an answer: it is written as `source: drafted, confirmed: false`, the gates treat it as unanswered, and accepting writes a *new* row so the proposal stays underneath. Every quote is re-verified verbatim against the stored document on the web side, not only by the agent — the agent's check protects the wire, this one protects the record. A proposal may only offer a value the field actually accepts. |
| S12 | **Handoff summary & risk scenarios** | Writes the reviewer's summary of a submitted assessment and proposes risk scenarios worth asking about, inferred across everything the assessment holds — how the activity was described, what data it touches, the controls and their findings. Each scenario is filed to the risk domain whose answers it cites. | A scenario is a **question, never a finding** (§4.4): there is no field in which acceptance could be recorded, and it is dropped rather than shown if it cites anything not in the record. The filing is deterministic — `control-domains.json` is versioned data a human ratified, so which domain sees a scenario is never a model's opinion. The summary sits above facts already derived from answers; it delays nothing. |
| S12 | **Risk-area explainer** | Says why a risk area applies to this activity and what its questions are getting at, in the words on screen rather than in the instrument's vocabulary. | It explains what is already true; it never proposes an answer, and it cannot change which areas apply. Silence is a valid result — an explanation it cannot ground is not offered. |
...
**The engine choice was open and under standing assessment (settled by G-53: Postgres on RDS).** Postgres is the Phase-1 implementation, not a settled destination. Two obligations follow: (a) no slice may adopt a store-specific feature without flagging it in its review as a constraint on the choice; (b) the implementer reports on the choice at the checkpoints in §14.6 with evidence from the data model as it actually exists, not from preference.
...
## 28. Control frameworks and crosswalks (G-80)

An enterprise does not ask "are your controls good"; it asks "which of my
obligations do they satisfy, and what is left". That is the crosswalk, and
it is only worth having if every reference in it is real.

1. **Framework content is transcribed, never remembered.** Each framework is
   a versioned file under `src/data/reference/frameworks/`, produced from
   the publisher's own machine-readable source by
   `scripts/fetch-framework.mjs`. The file records the source URL, the
   format, the retrieval date, the edition, and who checked it. A clause
   number nobody transcribed is a fabrication, and it is invisible to every
   other check in this repository.

2. **Licensed text is never redistributed.** A framework declares whether it
   may be quoted. A quotable one carries the published wording; a licensed
   one carries reference and heading only, and an item with text on it fails
   the build. The product ships public frameworks. An organisation loads the
   ones it is licensed for, by the same validation (FR-54) — which is also
   how it loads its own internal control set.

3. **A framework nobody has loaded says so.** Absence is a declared boundary
   on screen (§24.8), never a silent gap that reads as coverage.

4. **A mapping resolves at both ends or the build fails.** Every crosswalk
   entry names a control this instrument knows, a requirement the loaded
   framework contains, a relationship — **equivalent**, **partial** or
   **supporting** — and a reason. Partial is expected to be the common case;
   a boolean would flatter the mapping.

5. **The published wording is the basis.** Wherever the licence allows it,
   the requirement's own text is shown beside the mapping, and a test refuses
   a basis that does not appear in the loaded source. A mapping to a
   withdrawn requirement is named as out of date rather than counted.

6. **Coverage is computed, never asserted** (FR-53): the requirements no
...
7. **Not reached is not the same as a gap.** A regulation also defines terms
   and creates institutions, and no control can satisfy an article like that.
   Which requirements a control could satisfy is a judgement, recorded as one
   or the report overstates the gap. **Owed:** not made yet, and until it is
