...
- **G-58 (open — the only open defect in this document):** Client-side navigation is broken in production builds (2026-08-23). Clicking a link between assessment screens fails roughly four times in five: measured dev 15/15, `next start` 3/15, `output: standalone` served directly 3/15. The browser aborts the router's RSC request (`net::ERR_ABORTED`) while the server returns a valid 200 `text/x-component` payload, React hydrates, and full page loads work everywhere. Not caused by standalone output (removed and retested) and not by prefetching (disabling it made the rate worse). **Consequences:** the demo runs from `pnpm dev`, and §6.4's Tier-1 lift cannot proceed until this is understood — a container serving a production build is exactly the failing configuration. Recorded as open rather than worked around, because the workaround is a development server and that is not a deployment.
