...
| **S12** | Phase 2 · the agent service | not started | Its own image and Express Mode service; model access behind Bedrock; OpenTelemetry from its first day (§6.4 obligation 5); the `draft` capability with its eval. | §7, §22.1 | Verbatim quotes verified against source; full abstention scored correct. |
