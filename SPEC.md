...
- The residual starts at the inherent band and moves by whole bands, each step naming its reason; it never leaves the scale.
- The edition is refused at import for a rule with no band, a step that is not a whole number of bands, an empty reason, an appetite line on a scope that does not exist, or one that escalates to nobody.
