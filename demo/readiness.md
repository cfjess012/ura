...
- ~~The agentic layer is designed and registered, not built.~~ **Out of date as of 2026-08-24 — it is built and it runs.** What to say instead: five capabilities ship behind the seams (assistant, document drafting, policy authority and breach findings, the handoff report, intake scoring), and **every one of them is off by default**. With no agent connected the product is complete and says so; the assistant is absent rather than apologetic, and the intake assistant passes. Whether the demo shows them is a decision on the day — `AGENT_TRANSPORT`.
...
- ~~A reviewer cannot yet act.~~ **Out of date — S8 shipped.** The reviewer queue, attestation with the keyboard loop, server-side authority by risk area and the four dispositions all exist and are walked by the E2E suite.
...
## Phase 2, as it actually stands (2026-08-24)

Added because the two struck-through lines above were being read aloud after
they stopped being true, which is the exact failure §24.8 is about.

| Capability            | State                       | What to say                                                                                                                          |
| --------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Assistant (companion) | Built, off by default       | It helps a person think. It never answers for them, and it is checked against that assessment's record before it says anything.      |
| Document → proposals  | Built, off by default       | It proposes; the person accepts. A proposal counts as an answer nowhere until they do, and every one carries a verbatim quote.       |
| Policy authority      | Built, **no agent needed**  | Every control question cites the clause requiring it, quoted. Deterministic — it works with the agent off.                           |
| Breach findings       | Built, **no agent needed**  | An answer that breaches a clause reaches the reviewer with both quotes side by side, and settles through the same four dispositions. |
| Handoff report        | Built; agent adds two parts | Derived entirely from the record. The summary and the scenarios are the agent's; the report is complete without them.                |
| Intake scoring        | Built, off by default       | Suggestions beside the box. **It never blocks** — that is the whole design.                                                          |

**Not built:** packaging and export (S9), hardening (S10), and anything that
writes to a destination system.

**Never deployed.** `deploy/READINESS.md` is honest about what that means.
